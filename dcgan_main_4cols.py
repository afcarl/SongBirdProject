...
class _netG(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(ngf * 8, ngf * 4, kernel_size=(4,3), stride=(2,1), padding=(0,1), bias=False),
            nn.BatchNorm2d(ngf * 4),
            nn.ReLU(True),
            # state size. (ngf*4) x 10 x 4
            nn.ConvTranspose2d(ngf * 4, ngf * 2, kernel_size=(4,3), stride=(2,1), padding=(1,1), bias=False),
            nn.BatchNorm2d(ngf * 2),
            nn.ReLU(True),
            # state size. (ngf*2) x 20 x 4
            nn.ConvTranspose2d(ngf * 2,     ngf, kernel_size=(5,3), stride=(3,1), padding=(1,1), bias=False),
            nn.BatchNorm2d(ngf),
            nn.ReLU(True),
            # state size. (ngf) x 60 x 4
            nn.ConvTranspose2d(    ngf,      nc, kernel_size=(4,3), stride=(2,1), padding=1, bias=False),
            nn.Tanh()
            # state size. (nc) x 120 x 4
        )
...
class _netD(nn.Module):
    def __init__(self, ngpu):
        ...
        self.ngpu = ngpu
        self.main = nn.Sequential(
            # input is (nc) x 120 x 4
            nn.Conv2d(nc, ndf, kernel_size=4, stride=(2,1), padding=1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf) x 60 x 3
            
            nn.Conv2d(ndf, ndf * 2, kernel_size=(4,3), stride=(2,1), padding=1, bias=False),
            nn.BatchNorm2d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf*2) x 30 x 3
            
            nn.Conv2d(ndf * 2, ndf * 4, kernel_size=(4,3), stride=(2,1), padding=(2,1), bias=False),
        ...
        if (i % logpt == 0) & (i > 0):
            print('[%d/%d][%d/%d] Loss_D: %f (%f) Loss_G: %f (%f)'
                  % (epoch, opt.niter, i, len(dataloader),
                    np.mean(minibatchLossD[i-logpt:i]), np.mean(minibatchLossG[i-logpt:i]),
                    np.std(minibatchLossD[i-logpt:i]),  np.std(minibatchLossG[i-logpt:i]) ))
